...
M.on_attach.keybinds = function(e)
  ...
  vim.keymap.set("n", "K", function() vim.lsp.buf.hover() end,
    { buffer = e.buf, desc = "Displays information about the symbol under cursor" })
  vim.keymap.set("n", "<leader>vws", function() vim.lsp.buf.workspace_symbol()() end,
    { buffer = e.buf, desc = "List all symbols in current workspace in quickfix window" })
  vim.keymap.set("n", "<leader>vd", function() vim.diagnostic.open_float() end,
    { buffer = e.buf, desc = "Show diagnostics in floating window" })
  vim.keymap.set("n", "<leader>vca", function() vim.lsp.buf.code_action() end,
    { buffer = e.buf, desc = "Selects a code action available at the current cursor position" })
  vim.keymap.set("n", "<leader>vrr", function() vim.lsp.buf.references() end,
    { buffer = e.buf, desc = "List all references to the current symbol under the cursor in the quickfix window" })
  vim.keymap.set("n", "<leader>vrn", function() vim.lsp.buf.rename() end,
    { buffer = e.buf, desc = "Renames all references to the symbol under the cursor." })
